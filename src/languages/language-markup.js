...
export default  PrismLive.registerLanguage("markup", {
	comments: {
		multiline: ["<!--", "-->"]
	},
	...
	snippets: {
		...
		custom: function (selector) {
			...
			if (isName && inTag) {
				...
				var tag = PrismLive.match(selector, /^[\w:-]+/, "div");
				var html = `<${tag}`;
				var id = PrismLive.match(selector, /#([\w-]+)/, 1);
				...
				var selfClosing = PrismLive.languages.markup.selfClosing.indexOf(tag) > -1;
